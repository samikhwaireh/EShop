...
public class CartService : ICartService
{
    ...
    public async Task<CartResponse> GetCartByUserId(int userId)
    {
        var cart = await GetExistingOrCreateNewCart(userId);
        var cartResponse = AppMapper.Mapper.Map<CartResponse>(cart);
        ...
        if (cart.Items.Any(c => c.Product == null))
        {
            ...
            {
                var cartItemModel = AppMapper.Mapper.Map<CartItemResponse>(item);
